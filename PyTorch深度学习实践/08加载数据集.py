...
dataset = DiabetesDataset('D:\wenjian\py\pytorch-master\PyTorch深度学习实践\data\diabetes.csv.gz')
...
#采用上面一行计算损失会报错，为python不同版本进行更新迭代时引起的警告，某些参数被取缔了。改为下面一行即可。
criterion = torch.nn.BCELoss(reduction='mean')

#同理：criterion = torch.nn.BCELoss(size_average=False) 改为：
...
if __name__=='__main__':#if这条语句在windows系统下一定要加，否则会报错
    for epoch in range(20):  # 1000个epoch ，学习时可以写小点
